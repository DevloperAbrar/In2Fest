...
export default function SlotForm({ existingSlot, existingSlots = [], onSubmit, onCancel, submitting }) {
  ...
  const selectedSlugs = venue?.venue_type || [];
  ...
  const serviceTypeOptions = (categories || [])
  ...
  const handleFormSubmit = (values) => {
    ...
    onSubmit({
      ...
      name: chosen?.label || values.service_type,
      total_units: Number(values.total_units) || 1
    });
  };
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-5">
      ...
      <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
        ...
        <Button type="submit" loading={submitting} disabled={!selectedServiceType && serviceTypeOptions.length > 0}>
